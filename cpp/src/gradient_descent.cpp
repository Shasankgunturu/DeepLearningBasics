...
int main() {
    ...
    for (int epoch=0; epoch<iters; epoch++) {
        std::vector<double> y_predicted = model.forward(x_train, weight);
        double l = model.loss(y_predicted, y_train);
        double dw = model.gradient_descent(y_predicted, y_train, x_train);
        weight = weight - learning_rate*dw;
        if(epoch%10==0) {
            std::cout << "epoch " << epoch << ": w = " << weight << ", loss = " << l << std::endl; 
        }
    }
    std::cout << "prediction after training: f(9) = " << model.forward(9, weight) << std::endl;
